...
class History:
    ...
    @staticmethod
    def check(df, nf_func):
        old_links, old_titles = History.__read(Times.history_timedelta)
        df.drop(History.__links_check(df, old_links), inplace=True)
        _history_df = df.loc[:, ['Заголовок', 'Ссылка']]
        print(f'Новых строк для обновления истории: {len(df.index)}')
        time.sleep(0.1)
        df.drop(History.__titles_check(df, nf_func, old_titles),
                inplace=True
                )
        return df, _history_df

    @staticmethod
    def __read(delta: timedelta) -> tuple[set[str], set[str]]:
    ...
    @staticmethod
    def __titles_check(df, func, old_t):
        to_drop = set()
        for index, title in tqdm(list(
                zip(df.index, df['Заголовок'].values)),
